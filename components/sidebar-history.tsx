	...
	const hasEmptyChatHistory =
		paginatedChatHistories && Array.isArray(paginatedChatHistories)
			? paginatedChatHistories.every(
					(page) => page && Array.isArray(page.chats) && page.chats.length === 0
			  )
			: true;
	...
	const allChats =
		paginatedChatHistories && Array.isArray(paginatedChatHistories)
			? paginatedChatHistories.flatMap((page) =>
					page && Array.isArray(page.chats) ? page.chats : []
			  )
			: [];
					...
					if (chatHistories) {
						return chatHistories.map((chatHistory) => ({
							...
							chats: chatHistory.chats.filter(
								(chat: { id: string | null }) => chat.id !== deleteId
							),
						}));
					}
				});
				...
				<SidebarGroupContent>
					<SidebarMenu>
						{paginatedChatHistories &&
							(() => {
								// Use the same safe groupedChats that we calculated earlier
