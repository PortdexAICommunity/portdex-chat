...
const PureProductSearchResults = ({
  ...
  const [filters, setFilters] = useState<{
    ...
    minMOQ: args?.minMOQ,
    verifiedSuppliersOnly: args?.verifiedSuppliersOnly,
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600">
              ...
              {args.minPrice && ` • Min Price: ${args.minPrice}`}
              {args.minMOQ && ` • Min MOQ: ${args.minMOQ}`}
              {args.verifiedSuppliersOnly && ` • Verified Suppliers Only`}
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="p-4 border-b border-border">
          <CollapsibleFilterPanel 
            ...
            appliedFiltersCount={
              ...
              (args?.maxPrice ? 1 : 0) +
              (args?.minMOQ ? 1 : 0) +
              (args?.verifiedSuppliersOnly ? 1 : 0) +
              (args?.minRating ? 1 : 0)
            }
          />
        </div>
        ...
        <div className="p-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-96 overflow-y-auto no-scrollbar">
            {products.map((product) => (
              <div
                key={product.id}
                className="border border-border rounded-lg p-4 hover:shadow-md transition-shadow duration-200 bg-white dark:bg-neutral-800"
              >
                {/* Product Header */}
