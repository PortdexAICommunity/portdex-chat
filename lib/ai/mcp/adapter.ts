...
export const fetchLiveProductListings = async (
	...
	try {
						...
						const item: NormalizedProductItem = {
							...
							buyerInterest:
								typeof p?.watchers === "number" && p.watchers >= 0
									? p.watchers
									: undefined,
							moq: typeof p?.moq === "number" && p.moq >= 0 ? p.moq : undefined,
		...
		const filteredProducts = mapped.filter((product) => {
			...
			if (
				req.filters?.minPrice !== undefined &&
				product.price < req.filters.minPrice
			) {
				return false;
			}
			if (
				req.filters?.maxPrice !== undefined &&
				product.price > req.filters.maxPrice
			) {
				return false;
			}
			...
			// Apply MOQ filter
			if (
				req.filters?.minMOQ !== undefined &&
				product.moq !== undefined &&
				product.moq < req.filters.minMOQ
			) {
				return false;
			}
			...
			if (
				req.filters?.verifiedSuppliersOnly === true &&
				(!product.trustMetrics ||
					!product.trustMetrics.verifiedBadges ||
					product.trustMetrics.verifiedBadges.length === 0)
			) {
				return false;
			}
			...
			// Apply rating filter
			if (
				req.filters?.minRating !== undefined &&
				product.rating !== undefined &&
				product.rating < req.filters.minRating
			) {
				return false;
			}
